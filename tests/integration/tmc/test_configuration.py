...
def check_eda_instance():
    """Verify that the EDA DB is running."""
    pass


@given("an EDA configuration service")
def check_eda_service():
    """Verify that the EDA service is running."""
...
def configuration_file():
    """Write the EDA configuration file for this test."""
    with open(
...
def configure_archiver():
    """Configure the EDA and verify that it is healthy."""
    eda_es = con_config.get_device_proxy(EVENT_SUBSCRIBER)
...
def check_archived_attribute(
    ...
    """
    Verify that the obsState event is archived.

    :param sut_settings: SUT settings
    :type sut_settings: SutTestSettings
    :param context_monitoring: Context monitoring
