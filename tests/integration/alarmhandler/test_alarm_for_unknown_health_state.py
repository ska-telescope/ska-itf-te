...
from tests.integration.tmc.conftest import wait_for_attribute_value, wait_for_event
...
def check_alarm_state(response_data):
    ...
    assert wait_for_event(
        alarm_handler, "alarmUnacknowledged", tuple(alarm_tag), print_event_details=True
    )
    assert wait_for_attribute_value(alarm_handler, "alarmUnacknowledged", tuple(alarm_tag))
