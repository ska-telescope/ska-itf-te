...
ICAMS_PARAMS =  --set ska-icams-alarmhandler.backend.config.tango_host=$(TANGO_HOST) \
				...
				--set ska-icams-alarmhandler.scheduler.config.mongo_db_host=sut-$(CI_PIPELINE_ID)-mongodb.$(KUBE_NAMESPACE).svc.$(CLUSTER_DOMAIN) \
				...
				--set ska-icams-alarmhandler.backend.config.mongo_db_host=sut-$(CI_PIPELINE_ID)-mongodb.$(KUBE_NAMESPACE).svc.$(CLUSTER_DOMAIN) \
				--set ska-icams-alarmhandler.frontend.config.icams_api=http://sut-$(CI_PIPELINE_ID)-backend.$(KUBE_NAMESPACE).svc.$(CLUSTER_DOMAIN):3010 \
...
k8s-pre-install-chart:
	@echo "k8s-pre-install-chart: applying the ICAMS secrets file"
