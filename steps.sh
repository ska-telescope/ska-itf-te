...
# BEFORE poetry shell:
root@30ce45f0f0aa:/build/ska-telescope/ska-mid# env | grep PATH
...
# AFTER poetry shell:
(ska-mid-py3.10) root@30ce45f0f0aa:/build/ska-telescope/ska-mid# env | grep PATH
PYTHONPATH=/app/src:/app/src:/app/.venv/lib/python3.10/site-packages
PATH=/build/ska-telescope/ska-mid/.venv/bin:/app/.venv/bin:/app/bin:/app/.venv/bin:/app/.local/bin:/app/bin:/app/.local/bin:/app/.local/bin:/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin

# INSTEAD OF poetry shell, run:

# Once the shell is active, install everything
poetry install && export PATH=/build/ska-telescope/ska-mid/.venv/bin:${PATH}
