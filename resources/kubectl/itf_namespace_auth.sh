...
check_namespaces() {
  for NS in "${NAMESPACES[@]}"; do
    if [[ ${NS} != ci-${CI_PROJECT_NAME}-${CI_COMMIT_REF_SLUG}* ]] && \
       [[ ${NS} != ci-${CI_PROJECT_NAME}-${CI_JOB_ID}* ]] && \
       [[ ${NS} != ci-${CI_PROJECT_NAME}-${CI_COMMIT_SHORT_SHA}* ]]; then
      ERR_MESSAGE="Namespace is not following an expected format: ci-<project_name>-<branch name>*, ci-<project_name>-<gitlab job id>*, ci-<project_name>-<commit short SHA>* actual: $NS, expected: one of [ci-${CI_PROJECT_NAME}-${CI_COMMIT_REF_SLUG}*, ci-${CI_PROJECT_NAME}-${CI_JOB_ID}*, ci-${CI_PROJECT_NAME}-${CI_COMMIT_SHORT_SHA}*] $"
      exit 1
    fi
