...
Freq = 9e3                 # Start frequency Minimum 9kHz     
...
class sig_sock(socket.socket):
    ...
    def setSigGenStartFreq(self,freq_start=100e6):
        ...
        """
        self.sg_sendcmd(f'FREQ {freq_start}')                 # Set frequency
        data = int(self.sg_requestdata('FREQ?'))              # Query frequency
        print(f"Sig Gen START frequency = {(data/1e6)} MHz")  # Display received frequency
        return freq_start 
    ...
    def setSigGenStopFreq(self,freq_stop):
        ...
        """
        self.sg_sendcmd(f'FREQ {freq_stop}')                          # Set frequency
        data = int(self.sg_requestdata('FREQ?'))                      # Query frequency
        print(f"Sig Gen STOP frequency = {(data/1e6)} MHz")          # Display received frequency
        return freq_stop             
    ...
    def setSigGenDscrtFreqs(self, freq_start,freq_stop,freq_step):
        ...
        cur_freq = 0                                        # Initialize current freq container     
        i = 0
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description = "Specify Sig Gen Start Freq, Stop Freq, Step Freq, Power and turn on RF Output")
    ...
    sigGen.sig_gen_connect((sigHOST,sigPORT))    
